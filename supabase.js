...
async function dbGetPackage(id) {
  ...
  if (pkg && typeof pkg.questions === 'string') {
    try { pkg.questions = JSON.parse(pkg.questions); } catch {}
  }
  if (pkg && !Array.isArray(pkg.questions)) {
    pkg.questions = [];
  }
  return pkg;
}

// Те саме для dbLoadPackages — нормалізуємо questions
const _origLoad = dbLoadPackages;
